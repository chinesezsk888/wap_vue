<template>
  <div class="mypage consumers_complaint" >
    <div class="myheader">
      <div class="banner" ref="banner">
        <div @click="gohome" class="gohome"></div>
      </div>
      <!--nav -->
      <app-nav
    ...
    >
      ...
      <template v-if="emptypanel">
        <div class="consumers_complaint_empty">
    ...
    <div class="myfooter">
      <div class="btns">
        <div class="btn" @click="showAbout">关注我们</div>
        <div class="btn complaint_btn" @click="showReply">我要投诉</div>
        <div class="btn" @click="goAllPage">全部报道</div>
      </div>
    </div>
...
import TheReplyPanel from './components/TheReplyPanel'
import showAboutUs from './components/TheAboutUs'
import { sendH5APP, getUrlKey } from '@/util/common.js'
import loading from '@/util/loading'
import msgAlert from '@/util/message'
import {
  apiUrl, allPageHref, homePageHref, allPageId
} from './config.js'
...
export default {
  ...
  methods: {
    ...
    goAllPage () {
      if (this.AppStyle == 0) {
        this.mywindow.location = allPageHref
      } else {
        sendH5APP('22|' + allPageId)
      }
    },
    gohome () {
      if (this.AppStyle == 0) {
        this.mywindow.location = homePageHref
      } else {
        sendH5APP('1')
      }
    },
    showReply () {
      let appV = this.mywindow.$('#appV').val()
      if (this.AppStyle == 0 || appV <= '7.1.0') {
    ...
    showAbout () {
      showAboutUs(document.documentElement.scrollTop + document.body.scrollTop)
      this.limitScroll(true)
    },
    popupClose () {
      this.limitScroll(false)
...
/****/
  ...
  .consumers_complaint{
    .banner{
      ...
      height:1.36rem;
      position: relative;
      background: url(~@/assets/consumers_complaint/cc_banner.png) no-repeat;
      background-size: 100%;
      background-size: 100%;
    ...
    .complaint_btn{
      ...
      &::before{
        ...
        content:"";
        display:block;
        background:url(~@/assets/consumers_complaint/icon_reply.png) no-repeat;
        background-size: 80%;
        background-position: center;
        left: 0.48rem;
      }
      ...
      padding: 0.14rem 0.56rem 0.14rem 0.89rem;
    }
  }
  .consumers_complaint_empty{
    text-align:center;
