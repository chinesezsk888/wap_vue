...
<transition name="fade">
    <div class="nav_list" v-show="menuShow">
      <div v-for="item in itemList" class="nav_item" :class="{on:item[propertyId]==selectId}" :key="item[propertyId]" @click="selectList(item)">
        {{item[propertyTxt]}}
      </div>
    </div>
</transition>
  ...
  .nav_scroll{
    ...
    .menu_btn{
      ...
      .show{
        ...
        height: 0.32rem;
        display: block;
        background:url(~@/assets/consumers_complaint/icon_menu.png) no-repeat;
        background-size: 100%;
        margin: auto;
      }
      .close{
